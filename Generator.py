...
encoded_data_lp = simply_gender(encoded_data_lp,"Gender ")
...
# BIP
prob = LpProblem("UABC_Social_Event_Problem", LpMinimize)
...
prob += lpSum([encoded_data_lp.loc["Badminton Skill Level ", index] * variables[index] for index in variables]) <= 30 # Limit number of advance players per group

prob += lpSum([encoded_data_lp.loc["Which Club are you a member of?", index] * variables[index] for index in variables]) <= 30 # Even number of club members per group
for j in range(1,7):
    # Solve problem
