...
# Import Data
raw_data = get_data('C:/Users/victo/OneDrive/Desktop/Group Gen/UMSAV2.csv')
...
# Subject to:
prob += lpSum([variables[index] for index in variables]) <= 8 # Number of people per group
...
#prob += lpSum([encoded_data_lp.loc["Badminton Skill Level", index] * variables[index] for index in variables]) <= 25 # Limit number of advance players per group

prob += lpSum([encoded_data_lp.loc["Which Club are you a member of? ", index] * variables[index] for index in variables]) <= 6 # Even number of club members per group

prob += lpSum([encoded_data_lp.loc["Which Club are you a member of? ", index] * variables[index] for index in variables]) >= 2
...
for person in df:
    if person not in copy_df["Full_Name"]:
        print(person)

print(df)
df.to_csv('group_UMSA.csv', index=True)
print(([encoded_data_lp.loc["Gender", index] for index in variables]))
